...
- **Offline operation**: nothing here changes the offline-first principle (ADR-0004). Freshness signals are *opt-in network*; if the user is offline, `is_ready()` still returns true and `analyze` works against the local cache.
